...
bool variable_table::add_ind(string ind_name)
{
	...
	if (!check)
	{
		my_table[ind_hash].push_back(lexeme(ind_name));
		return true;
	}
	else
		return false;
...
bool variable_table::set_ind_type(std::string ind_name, int type)
{
	...
	{
		int ind_num = find_in_vector(ind_hash, ind_name); //looking for our element
		my_table[ind_hash][ind_num].type = type;
		return true;
	}
	else
		return false;
...
bool variable_table::set_ind_val(std::string ind_name, bool val)
{
	...
	{
		int ind_num = find_in_vector(ind_hash, ind_name); //looking for our element
		my_table[ind_hash][ind_num].value = val;
		return true;
	}
	else
		return false;
...
{
	if (hash >= 0 && hash < hash_N && numb >= 0 && numb < my_table[hash].size())
	{
		my_table[hash][numb].type = type;
		return true;
	}
	return false;
}
...
bool variable_table::check_ind_in_table(string ind_name)
{
	...
	int num = find_in_vector(ind_hash, ind_name); //number of elem
...
int variable_table::find_in_vector(int hash, string ind_name)
{
	...
	for (int i = 0; i < my_table[hash].size(); i++)
		if (find_t == my_table[hash][i])
			return i;
	return -1;
}
...
bool variable_table::get_lexeme(string ind_name, lexeme& gets_lexeme)
{
	...
	if (check) {
		gets_lexeme = my_table[hash][numb];
		return true;
	}
	return false;
}
